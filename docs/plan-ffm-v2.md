...
したがって **FFM コードのコンパイルとテストは開発機（Windows） で行う**（`gradlew test`）。Claude はコードを書き、
...
### P0. 準備（開発機（Windows） で 0.5 セッション）— **完了 2026-09-07**
...
> 素材は実際の会議録音のため、ファイル名は `sample-b` などに置き換え、固有名詞は「姓A」「姓B」と
> 匿名化してある（同じ記号は同じ人物）。数値は実測値そのまま。
...
未実測なのでアプリ側の既定も変えなかった（今回の教訓の適用）。

#### その A/B の結果（2026-09-08）— **`carryInitialPrompt = true` は採用しない**

同一音声（sample-c、12 分 38 秒、small、VAD off、参加者名のプロンプトあり、Windows i5-1335U）で 3 条件。

| 条件 | 文字数 | 行数 | 処理時間 | 3 行以上の繰り返し | 固有名詞（例: 姓A / 姓B） |
|---|---|---|---|---|---|
| 既定（`carry=false` / `bestOf=-1` / `temp_inc=0.4`） | 3,542 | 93 | 257 秒 | 0 行 | 2 / 0 |
| `bestOf=5` / `temp_inc=0.2` | 3,451 | 95 | 244 秒 | 0 行 | — |
| **`carry=true`** | 4,713 | 166 | 271 秒 | **42 行** | **7 / 1** |

`carry=true` は**固有名詞には効いた**（姓A 2 → 7、姓B 0 → 1）。予想どおり、初期プロンプトが毎ウィンドウ
前置されることで語彙への影響が音声全体に及ぶ。しかし同時に**初期プロンプト自身が出力へ漏れ出し**、
初期プロンプトを言い換えたような 1 行が 42 行連続した。文字数が 33% 増えたのは中身ではなくこの水増し。
議事録として読めなくなるので**採用しない**。既定（false）を維持する。

つまり、この機構は「プロンプトの影響力を上げる」ものであり、上げすぎるとプロンプトの復唱を招く。
繰り返しループの対策としては使えない。

未検証の中間案: プロンプトを「〜です。」という文ではなく名前の羅列だけにする、極端に短くする、
`maxTextContext` を小さめの正の値にして予算を静的プロンプトでほぼ埋める、など。
いずれも同一音声での A/B が前提。
